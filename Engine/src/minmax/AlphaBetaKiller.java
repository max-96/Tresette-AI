...
public class AlphaBetaKiller extends DeterministicAI
{
	...
	private double minmax(List<List<Integer>> oldassegnamentoCarte, int startingPlayer, List<Integer> oldcardsOnTable,
			Integer carta, int currPlayer, boolean maximise, double alpha, double beta, int depth, int currDepth,
			double oldScore)
	{
		...
		/*
		 * falsa foglia, ci sarebbe ancora da analizzare ma potiamo per ragioni di
		 * efficienza
		 * 
		 */
		...
		for (Integer mossa : mosse)
		{
			...
			double mmaxval = minmax(assegnamentoCarte, startingPlayer, cardsOnTable, mossa, currPlayer, !maximise,
					alpha, beta, depth, currDepth + 1, score);// chiama minmax
			...
			if (maximise)
			{
				...
				if (beta <= alpha)
				{
					...
					// System.out.println("alpha pruning\t" + alpha +
					// "\t"+mossa+"\t"+currPlayer+"\t"+(10-assegnamentoCarte.get(currPlayer).size()));
					break;
				}
			...
			} else
			{
				...
				if (beta <= alpha)
				{
					...
					// System.out.println("beta pruning\t" + beta +
					// "\t"+mossa+"\t"+currPlayer+"\t"+(10-assegnamentoCarte.get(currPlayer).size()));
					break;
				}
			}
